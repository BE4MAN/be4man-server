...
public class StageAnalysisService {
    ...
    public void analyzeFailedStages(List<StageRun> targets) {
        ...
        for (StageRun stage : targets) {
            try {
                ...
                log.warn("[StageAnalysis] stageRunId={} 분석 실패: {}", stage.getId(), e.getMessage(), e);
    ...
    private String buildPrompt(String stageName, String log) {
        ...
        return """
            ...
            3) Answer above questions only Korean and clearly split two answers.
            4) Don't generate text except for answers like emphasis.
            5) Do not use bolding (**...**) anywhere in the response, even for emphasis or headings.
